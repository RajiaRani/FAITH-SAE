"""_common.py — shared setup every step script in milestone 8 imports.

WHY THIS FILE EXISTS
--------------------
Each step script (step1_*.py ... step4_*.py) needs to do the SAME boring things
before it can start teaching:
...
The project root (three levels up from this file:
  .../25_Rajia_Rani_FAITH_SAE/code/milestone_8_analysis/_common.py
   parents[0] = milestone_8_analysis
   parents[1] = code
...
research code (the STEER_REGISTRY, cfs_score, the TopK SAE) instead of
re-implementing it.
"""
...
HERE = pathlib.Path(__file__).resolve().parent       # milestone_8_analysis/
...
def load_cfg(path: str = "config.yaml") -> dict:
    ...
    name->value pairs, e.g. cfg["n_boot"] -> 2000.
    """
...
def here_path(name: str) -> str:
    """Absolute path inside this milestone's folder (for files like FINDINGS.md
    that live next to the code, not in outputs/)."""
